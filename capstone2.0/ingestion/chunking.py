...
SEMESTER_PATTERN = r"SEMESTER[-\s]*(I|II|III|IV|V|VI|VII|VIII|1|2|3|4|5|6|7|8)"
...
def detect_semester(text: str):
    match = re.search(SEMESTER_PATTERN, text, re.IGNORECASE)
    if match:
...
def split_documents(documents):
    ...
    for i, match in enumerate(course_matches):
        start = match.start()
        end = course_matches[i + 1].start() if i + 1 < len(course_matches) else len(full_text)
